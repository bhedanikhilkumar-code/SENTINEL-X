...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="h-14 sm:h-16 bg-[#0c1220]/95 backdrop-blur-xl border-b border-cyber-border flex items-center justify-between px-3 sm:px-6 z-40 sticky top-0 select-none">
      {/* Left: Mobile Hamburger & Platform Branding */}
      <div className="flex items-center space-x-2 sm:space-x-3.5">
        {/* Mobile Hamburger Button */}
        <button
          onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
          className="p-1.5 rounded-lg bg-slate-900/90 border border-slate-700 md:hidden text-cyan-400 hover:text-white cursor-pointer transition active:scale-95"
        ...
        <div className="flex items-center space-x-2">
          <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-cyan-500/10 border border-cyan-500/50 flex items-center justify-center shadow-glow-cyan shrink-0">
            <Shield className="w-4 h-4 text-cyan-400" />
          </div>
          <div>
            <div className="flex items-center space-x-1.5 sm:space-x-2">
              <span className="font-mono font-bold text-xs sm:text-base tracking-wider text-white">SENTINEL-X</span>
              <span className="text-[8.5px] sm:text-[10px] uppercase font-mono px-1 sm:px-1.5 py-0.2 sm:py-0.5 rounded bg-cyan-500/20 text-cyan-300 border border-cyan-500/40">
                NTRO
              </span>
            </div>
      ...
      {/* Center: Mobile Target Badge / Desktop Status Row */}
      <div className="flex items-center space-x-1.5 sm:space-x-3">
        {/* Mobile-Only Active Target Chip */}
        <div className="flex md:hidden items-center space-x-1.5 px-2 py-0.8 rounded-lg bg-slate-950/80 border border-slate-800 text-[10px] font-mono">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
          <span className="text-cyan-300 font-bold truncate max-w-[90px]">
            {store.selectedCaseId?.includes('void') ? 'VOID-LCK' : 'PHANTOM'}
          </span>
        </div>

        {/* Backend / Cloudflare Tunnel Connection Badge (Desktop & Mobile) */}
        <button
          onClick={() => setShowTunnelModal(true)}
          className="flex items-center space-x-1 sm:space-x-2 px-1.5 sm:px-3 py-1 rounded bg-black/40 border border-cyber-border hover:border-cyan-500/50 text-[10px] sm:text-xs font-mono transition cursor-pointer"
        ...
        {/* CHECK 17: Tor Collector Status Badge (Desktop Only) */}
        ...
        <button
          ...
          className="flex items-center space-x-1 sm:space-x-2 px-2 sm:px-3 py-1 rounded bg-black/40 border border-cyber-border hover:border-cyan-500/50 text-[10px] sm:text-xs font-mono transition cursor-pointer"
          title="Click to view live alerts"
        >
          <Radio className="w-3.5 h-3.5 text-cyan-400 animate-pulse" />
          <span className="text-slate-300 hidden md:inline">LIVE:</span>
          <span className="text-cyan-400 font-bold">{currentAlerts.length}</span>
        </button>

        {/* SPECTER-AI Copilot Button (Desktop only, mobile has Floating FAB) */}
        <button
          onClick={() => {
            setShowAiCopilot(true);
            store.setIsAiCopilotOpen(true);
          }}
          className="hidden md:flex items-center space-x-1.5 px-2.5 sm:px-3 py-1 rounded bg-gradient-to-r from-cyan-500/20 to-blue-600/30 border border-cyan-400/80 text-cyan-300 hover:text-white text-xs font-mono font-bold transition shadow-glow-cyan cursor-pointer"
        ...
        {/* Darknet Ingestion Extractor (Desktop Only) */}
        <button
        ...
        {/* Court Subpoena Generator (Desktop Only) */}
        <button
      ...
      {/* Right: User / Authentication Badge (CHECK 19 & CHECK 20) */}
      <div className="flex items-center space-x-1.5 sm:space-x-3">
        {installPrompt && (
          <button
            onClick={handleInstallPwa}
            className="flex items-center space-x-1 px-1.5 sm:px-2.5 py-1 rounded bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/50 text-[9px] sm:text-xs font-mono transition cursor-pointer shadow-glow-cyan animate-pulse"
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-3 pl-1 sm:pl-3 border-l border-cyber-border">
          ...
          <button
            ...
            className="p-1.5 sm:p-2 rounded bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 transition-colors cursor-pointer active:scale-95"
          >
      ...
      <SpecterAiCopilot
        ...
        onTriggerAction={(actionId) => {
          ...
          if (actionId === 'open_subpoena' || actionId === 'open_subpoena_exchange') {
            ...
            sessionStorage.setItem('sentinel_pending_tab', 'map');
