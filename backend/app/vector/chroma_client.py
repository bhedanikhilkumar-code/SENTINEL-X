...
def is_chroma_server_available(host: str = CHROMA_HOST, port: int = CHROMA_PORT, timeout: float = 0.5) -> bool:
    """Fast socket probe to check if remote ChromaDB server is listening."""
    try:
...
class CustomStylometryEmbeddingFunction(chromadb.EmbeddingFunction):
    """Custom embedding function using Module C's dense 384D stylometric semantic vector."""
...
def get_chroma_client():
    """Obtain or initialize ChromaDB client with HttpClient -> PersistentClient fallback."""
    global _client
    ...
    # 2. Local PersistentClient fallback
    try:
